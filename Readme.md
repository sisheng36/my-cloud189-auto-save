...
### 6. AI 配置验证与 CAS 秒传自动化增强
- **(新) AI 配置调试工具**：在设置界面新增“测试连接”和“获取模型列表”按钮。支持一键验证 BaseURL 和 API Key 的有效性，并能自动从远端服务器抓取所有可用模型 ID。内置**模糊匹配搜索**，点击即可填入，告别手动输入长字符串的烦恼。
- **(新) CAS 秒传全流程整合**：修复了通过 `.cas` 文件秒传成功后无法触发后续流程的缺陷。现在，秒传成功的视频文件会与常规转存文件一样，自动触发 **AI 重命名、TMDB 刮削、STRM 生成及推送通知**，实现真正的全自动化管线。
- **(新) 后端请求稳定性优化**：全面迁移至 `got` 网络库并兼容 Node 18+ 原生请求机制，即便在复杂的网络代理环境下也能提供更精准的错误日志回显（包括详细的 API 返回错误正文）。
